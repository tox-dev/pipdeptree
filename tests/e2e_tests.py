...
def _test_cmp_with_file_contents(spec):
    p = subprocess.Popen(shlex.split(spec["command"]), stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    out, err = p.communicate()

    assert spec["expected_returncode"] == p.returncode

    if spec["expected_output_file"] is not None:
        exp_output_file = os.path.join(test_profile_dir, spec["expected_output_file"])
    ...
    if spec["expected_err_file"] is not None:
        exp_err_file = os.path.join(test_profile_dir, spec["expected_err_file"])
