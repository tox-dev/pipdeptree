...
def reexec_in_virtualenv_maybe():
    ...
    if abs_sys_executable != abs_venv_python:
        # Use -B to disable creation of bytecode files. Since the pipdeptree might
        # be run from virtualenvs with different Python versions, the bytecodes
        # generated by these might be incompatible
        sys.exit(subprocess.call([abs_venv_python, '-B', __file__] + sys.argv[1:]))
