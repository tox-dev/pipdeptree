        ...
        for node in self._obj.keys():
            ...
            while True:
                if len(stack) > 0:
                    ...
                    break
        assert not exclude & set([n.key for n in m])
        if exclude_re:
            _m = {}
            for n, l in m.items():
                if re.match(exclude_re, n.key) is None:
                    _m[n] = [ i for i in l if re.match(exclude_re, i.key) is None ]
            m = _m
...
def render_text(tree, list_all=True, frozen=False, output=None):
...
def print_graphviz(dump_output, output=None):
    ...
    if hasattr(dump_output, 'encode'):
        ...
        with os.fdopen(output.fileno(), 'wb') as bytestream:
...
def get_parser():
    ...
    parser.add_argument('--exclude-regex', dest='exclude_regex',
                        help=(
                            'Regex of select packages to exclude '
                            'from the output. If set, --all will be ignored.'
                        ), metavar='PACKAGES')
    ...
    parser.add_argument('-g', '--graph-output', dest='output_format',
                        help=(
...
def main():
    ...
    if show_only is not None or exclude is not None or exclude_re is not None:
        tree = tree.filter(show_only, exclude, include_re, exclude_re)

    output = sys.stdout
    try:
        if args.output:
            output = TextIOWrapper(open(args.output, 'wb'))
        if args.json:
            print(render_json(tree, indent=4), file=output)
        elif args.json_tree:
            jsontree = render_json_tree(tree, indent=4)
            print(jsontree, file=output)
        elif args.output_format:
            graph = dump_graphviz(tree,
                                  output_format=args.output_format,
                                  is_reverse=args.reverse)
            output = print_graphviz(graph, output=output)
        else:
            render_text(tree, args.all, args.freeze, output=output)
    except Exception as e:
        print(traceback.format_exception(e.__class__, e, e.__traceback__), file=sys.stderr)
        raise e
    finally:
        if args.output:
            output.close()
